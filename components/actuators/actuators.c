...
esp_err_t actuators_init(void){
    ...
    // configuration of operator, comparator and generator for each servo
    for(int i=0; i<SERVO_COUNT; i++){
        ...
        // configure generator action (HIGH at the beginning, LOW on match)
...
// set each servo at a neutral angle
esp_err_t actuators_set_neutral(void){
    const float neutral_angles[SERVO_COUNT]={NEUTRAL_ANGLE, NEUTRAL_ANGLE, NEUTRAL_ANGLE};
